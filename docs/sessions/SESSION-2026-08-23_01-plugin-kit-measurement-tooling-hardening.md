...
**State** (2026-08-24 afternoon, through `853237f`): everything on the published `main`. The morning close's landings stand (collector fixes, Skill grant, recall layer, recall-aware verdicts, the rewritten doctrine and operator references, the ToC standard across 25 files with its warn-tier rule, the genre detectors and catalog, the habit-flag removal). The afternoon added: the doctrine-propagation sweep across four homes (reviewer agents, skill-frontmatter, portability, the skill-creator body — the struck pointer-form rule and stale metrics gone everywhere, allowed-tools kept as a capability with its measured headless trap taught beside it); subset runs on both measurement CLIs, mechanically incomparable by set-hash and chipped in the dashboard alongside the previously unmarked tier-study class; the disclosure envelope — the last measurement tool without one — with the counted-versus-all cap and compareRuns working across disclosure arms; install detection distinguishing not-reachable from unknown, every consumer swept. ANALYSIS-006 (24 findings, weak-model routing: causes, remedies, detection, the named sources) created and integrated into the standards as evaluation criteria plus authoring doctrine; ANALYSIS-005 corrected at nine sites — rule 1's evidence withdrawn, and none of the four surviving structural rules now carries a positive measured result. Suite 1,719 passing. Open: the isolation audit with the per-run proof (in flight); candidates on record — instance-aware worker default, fail-fast on a broken grant, embedding-overlap detector, inert-scenario audit, installed-set reporter, cross-tier diff column, the plugin-creator body 12 tokens over, the five reviewer descriptions past 1,024, the suite's uncaptured intermittent red.
...
## Event — 2026-08-24 afternoon: the doctrine-propagation sweep across four homes

- Timestamp: 2026-08-24 13:35 PDT, measured. Orchestrated from the companion ask-user-question session (its Events 123-157 carry the full narrative); recorded here because every change lands in this repository
- `f88fb50` — the four component reviewer agents re-grounded in current doctrine: the struck pointer-form rule replaced with the strike stated plus evidence (one historicizing clause per file, zero teaching hits), skill-reviewer gaining the ToC check in validator terms and measured-evidence steering, mcp-reviewer's surviving schema-deferral finding split out as Major; skill-frontmatter.md keeps allowed-tools as a capability and teaches the measured headless trap with the upstream issue numbers and the Execute-versus-Launching diagnostic
- `61befe4` — portability.md's blanket "Add these freely" qualified in the same breath it is stated; classification affirmed before the exception
- `6c0bc4e` — the skill-creator body's three stale lines fixed under its 5,000-token ceiling (4,998 to 4,990, deletions sequenced before additions): review routing repointed to plugin-kit:skill-reviewer, recall named as the disclosure evidence with pull rate demoted-but-retained, the struck clause removed; the authoring checklist's fallback line repointed after verifying the standalone plugin ships its own copy
- Owner rulings behind the sweep: allowed-tools stays a supported field (no ban, no strip — the harness grant already covers measurement); the reviewer class was found stale by owner suspicion, and a review mid-run on the stale definition was stopped and re-dispatched rather than filtered

## Event — 2026-08-24 afternoon: subset runs, and the envelope closes the last gap

- Timestamp: 2026-08-24 13:35 PDT, measured
- `1600348` — subset selection on both measurement CLIs per owner ask: --only by index, scenario id, or group-via-sidecar; unknown selectors fail loudly; a subset is MECHANICALLY incomparable (evalSetHash computed over the rows that ran, so compareRuns refuses slice-versus-full and slice-versus-slice); the CLI shows only filtered rows with subset denominators; the dashboard chips BOTH not-of-record classes (subset and the previously unmarked tier-study) from the heartbeated status files; operator docs gained iterating-on-a-slice subsections. Suite 1,612 to 1,688
- `c78e6b3` then `3819d55` — the enforcement asymmetry first stated honestly (the disclosure stamp was a statement, not a guard), then erased by the build: measure-disclosure, the last measurement tool without an envelope, now writes one by reusing the shared builder with optimizer-only caps gated per operation; the counted-versus-all cap names both denominators and every exclusion cause, closing the interrupted-pair improvement candidate; T-13 done — install state distinguishes not-reachable (blind sweep) from unknown (no sweep), blindness downgrades absent and installed but never shadowed, and measure-outcomes gained the conflict check it never had. Suite 1,688 to 1,719
- ESCALATION HELD then SUPERSEDED by owner reframe: the install sweep is structurally blind to the plugin cache (five copies of the measured skill live there), so historical install_state absent was unearned — the owner redirected from record-the-isolation to GUARANTEE it, and an isolation audit with a per-run proof is in flight (the sole open thread at this event)

## Event — 2026-08-24 afternoon: ANALYSIS-006, the nine-site correction, and the standards integration

- Timestamp: 2026-08-24 13:35 PDT, measured
- `b49a383` and `d4f2610` — ANALYSIS-006 (weak-model routing for progressive disclosure) created and expanded to 24 evidence-labeled findings: why weaker models under-route (vendor tool-count bounds, measured long-list degradation, polarised failure, compositional load), remedies (capability-graded benefit, the strong-rewrite-can-hurt result, the no-portable-phrasing convergence, the two-surface split), detection (cross-tier matrices, routing observability, the training-prior test), and the named sources including the Vercel/TanStack passive-context convergence and TanStack's scoped measurement negative. One fabricated-summary figure caught by the primary-text rule — second instance of that fault class
- `3ca48a1` — ANALYSIS-005 corrected at nine sites, dated: the Vercel citation measured host-prompt forced invocation in passive context, not a numbered workflow in a body; rule 1's verdict is SURVIVES, EVIDENCE WITHDRAWN, kept distinct from refutation; after the correction none of the four surviving structural rules carries a positive measured result, and the note says so. New discipline: a citation inherited from a sibling note counts as unverified until opened in primary text here
- `853237f` — the integration per owner directive: identification findings as evaluation criteria (sibling overlap, routing-tier measurement, no inert should-fire scenario, deliberate installed-set sizing with the transfer fence) and remedy findings as authoring doctrine, across five reference files plus the authoring checklist, fences verified (zero graph artifacts in skill content), skill-creator body untouched at 4,990. Four validator automations recorded as candidates, none built: embedding-overlap detector, inert-scenario audit over existing ablation output, installed-set reporter, cross-tier diff column
- Also this afternoon, in the companion project but measured through this toolkit: the trigger-description saga (separating run proving the post-record rewrite cost the weak tier ten phrases; the restore shipped as AUQ 0.1.5), and the AUQ repo published to GitHub

## Event — 2026-08-24: the isolation guarantee lands at `10622f0`, and the audit finds a second open channel

- Timestamp: 2026-08-24 14:38 PDT, measured. Orchestrated from the companion ask-user-question session; recorded here because the changes land in this repository
- The channel audit's headline: every eval child and grader this repository has ever spawned carried the operator's private auto-memory file inlined into its system prompt — roughly 200 lines of behavioural instruction in every measurement's context, unfenced because the auto-memory directory is not resolved through the setting-source allow-list. Probed live before believing (the child quoted the file's first heading back). Closed with the dedicated disable env var applied after the env merge; the bare-mode and HOME-redirect alternatives were probed and both break OAuth
- The cross-session channel's mechanism corrected by probe: children are not enrolled by inherited sockets — each binds its OWN inbox and registers itself, so env scrubbing severs nothing; denying the messaging tools does, and a bare name in disallowedTools removes the tool from the advertised list entirely. Outbound severed; INBOUND remains open with the upstream ask named (an opt-out shaped like the auto-memory env var, checked before the bare-mode gate)
- The per-run proof: zero extra spawns — the init event the harnesses already parse enumerates the child's whole surface; run.isolation is a REQUIRED four-way comparability key (verified / violated / unverified / not-applicable), hard checks that cannot false-positive split from a version-dependent soft caveat, first violation loud on stderr, violated runs keep their transcripts and lose their believability. Graders stay unprovable under text output — a named gap, closable by moving them to stream-json
...
## Event — 2026-08-24: round two of the readable editions replaces round one

- Timestamp: 2026-08-24 16:34 PDT, measured. Orchestrated from the companion session; the notes land here
- Owner pushed past round one with full structural latitude and a two-layer research mandate (human readability: plain-English, F-pattern inverted pyramid, worked examples, Diátaxis; agent readability: greppable stable anchors, head-survives-compaction leads, self-contained sections for chunked reads, one-word-one-concept as searchability). The first two round-2 dispatches were cancelled on owner instruction until the agent-layer research was done, then all three re-dispatched with complete briefs
- Landed at 947e80d, 441bb5f, be4e0ce, replacing the round-1 siblings (deleted, in git history) with plain-titled rebuilds: ANALYSIS-007 "Do Reference Files Get Read, and How to Tell", ANALYSIS-009 "Why Weaker Models Miss Skills, and What Helps", ANALYSIS-008 "The Fourteen Genres of Skill Content" — each conclusions-first, worked-example-bearing, uniformly shaped, parity-checked mechanically against its untouched note of record, with measured readability where claimed (007: median 15-word sentences, 81 percent at or under 25)
...
- relates_to [[ANALYSIS-008: The Fourteen Genres of Skill Content]]
- relates_to [[ANALYSIS-009: Why Weaker Models Miss Skills, and What Helps]]