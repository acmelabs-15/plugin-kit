...
**The one live hypothesis is placement, and it is not yet proven.** The best-performing reference is the only one pointed to from inside the numbered workflow steps; the worst are single pointers in trailing sections. Position is not the explanation, and it is worth being precise about why: those sections sit at lines 369-439 of 456, which is end-of-context and a favoured retrieval position, so the effect runs opposite to what position predicts. But mention count and placement are confounded in the current data, so this justifies an experiment rather than a rule.
...
**The honest limit: mention count and placement vary together across all six references.** The best differs from the worst in both at once, and nothing in the current data separates them. This justifies a controlled run, not a rule.
...
- **Whether placement causes the Finding 4 effect or merely correlates with it.** Mention count and placement vary together across all six references, and no external work separates them. Needs a controlled run that moves one pointer without changing its count.
...
3. **Test the placement hypothesis before writing it into any standard** (Finding 4). Move one trailing pointer inside the step that needs it, hold the mention count fixed, re-measure on sonnet. It has a correlation, one external result, and one filesystem-level analogue behind it — enough for an experiment, not for a rule.
...
- [problem] Mention count and placement vary together across all six references, so the placement result is confounded and justifies an experiment rather than a rule #confound #open
...
- [problem] Shipped practice spans zero to 66 reference files with no published rationale anywhere on that range, so the open question is not what the cap is but what splitting buys — which nobody has measured #open #unmeasured
