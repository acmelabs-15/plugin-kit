...
**A published mechanism is consistent with that null, and it says position was never the variable** (measured, external). Constraint Saturation Evaluation (arXiv 2608.12426) varied one to twelve simultaneous constraints across 15 models with deterministic rule-based verifiers: "per-constraint pass rate decays gradually and predictably, while the chance of satisfying all k constraints collapses — a model passing individual constraints at ~41% at k=8 succeeds on all eight just 5.7% of the time", and "Reliable instruction following breaks down beyond 5-6 simultaneous constraints". Read the placement arm through that: moving a pointer into a numbered step does not merely relocate it, it adds it to that step's constraint budget, and the conjunction is the term that collapses. It predicts the observed halving without any positional account at all.

Hold this as *consistent with* rather than *proven here*. The constraints that paper measures are verifier-checkable output properties — a schema, a word limit — and no published work extends the result to "read this file when X", so the application is an inference. What it does license is a change of instinct when a pointer is missed: **reduce what else the step asks for, rather than strengthening the pointer**. Strengthening one constraint raises the term that decays gently; only removing constraints touches the term that collapses. Vercel measured the same direction from the opposite end, finding "You MUST invoke the skill" beaten by an instruction that sequenced the work and named the skill second (measured, external; the post names no model, so its tier-sensitivity is unknown).
...
**Two teams have independently shipped that workaround, and one of them measured it.** Vercel put a task-to-file instruction into `AGENTS.md` — always-loaded passive context, authored outside the skill — and measured a compressed docs index there at a 100% pass rate against 53 to 79% for the same content behind a skill; the skill on its own scored no better than having no documentation at all until an explicit instruction was added, with "In 56% of eval cases, the skill was never invoked" (measured, external). TanStack ships the identical architecture as a mechanism rather than a finding: `npx @tanstack/intent install` writes an `intent-skills` block into the agent's own config, pairing a task description with a file path under the header "Skill mappings — when working in these areas, load the linked skill file into context" (shipped practice, no evaluation published). Vercel's own account of why it wins names three properties, and the third is the one this page cares about: "**Skills create sequencing decisions** (read docs first vs. explore project first). Passive context avoids this entirely."

Read that honestly in both directions. Two unconnected teams converging on the same structural answer is the strongest non-measured signal that in-artifact routing is the weaker architecture — and neither result is about this repository's skills, Vercel names no model anywhere so its tier-sensitivity is unknown, and TanStack publishes no evaluation at all. It is also not a lever a skill author holds: nothing inside a `SKILL.md` can install a line into the host's always-loaded context. What it does settle is the framing above. The pointer guidance on this page is compensation for a missing mechanism, not the natural way to author references, and a consumer who *does* control their own always-loaded context has a measured better option than any pointer wording.
