...
This plugin's own suites follow the rule rather than deviating from it: each function directory under `shared/` carries its own `__tests__/` beside the code it covers — `../validate/__tests__/`, `../operations/__tests__/`, `../report/__tests__/`, `../tools/__tests__/`, `../parse/__tests__/`, `../util/__tests__/`, `../schemas/__tests__/`. They were briefly pooled at one root while the layer was named by provenance, and the point at which that stopped paying was exactly the one described above: with the subjects spread across five directories, "which of these tests my file" was no longer obvious from a single listing.
