...
**Nothing enforces that here, and the asymmetry is worth knowing.** `measure-triggering.ts` writes a results envelope whose `run.evalSetHash` is computed over the rows that actually ran, and because `evalSetHash` is a comparability key, `compareRuns` mechanically refuses a delta between a subset and a full sweep — the tooling stops you. `measure-disclosure.ts` writes no envelope at all, so there is no hash and no such refusal: the stamp on a disclosure subset is a statement, not a guard. The discipline is yours to keep.
