...
**Signposting (Minor; Critical when the body needs a file it never names).** A pointer should carry the condition that makes the agent open it, not merely the file's topic. "Read `docs/schema.md` when the payload has a field you do not recognise" beats "see `docs/` for details": the second says the file exists, the first says when to reach for it. Report a topical pointer as Minor with the condition written out. A body that depends on a fact in a file it never names at all is Critical — the agent gets one prompt and cannot ask which file was meant.
