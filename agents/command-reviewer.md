...
description: |
  ...
  Do not use to write or edit a command — this agent is read-only and reports findings; the command-creator skill does the authoring. Do not use to review a skill whose value is Claude loading it automatically, or one carrying bundled scripts, references and assets — the skill-reviewer agent covers those. Do not use to review a subagent definition or an MCP server entry — agent-reviewer and mcp-reviewer cover those. Do not use to review a hook — no agent in this plugin covers hook review. Do not use for plugin-level manifest and layout validation — that is the plugin-reviewer agent, plus `claude plugin validate --strict`.
...
**Signposting (Critical when nothing points at the file; otherwise judgement, not a rule).** A bundled file nothing points at stays Critical (Section 7), and that half is unchanged — no condition can fire for a pointer that is not there.

The wording of a pointer that *does* exist is a different matter. The rule this review used to apply — name the file, the condition that fires it, and the cost of skipping it — is struck. It has no published basis, no analogue across the eight vendors surveyed, and the one measurement touching it recorded 33% to 75% recall on the weaker tier for the references carrying its fullest form, so following it did not prevent the failure it exists to prevent. Placement was tested directly and refuted: moving a single pointer into the numbered step where its condition fires halved reach, 8/40 against 4/40, p≈0.20. Reachability is unmeasured by form, and no harness measures it here — the disclosure sweep that would settle it runs against a skill's bundled files and its scenario set.

So the checkable part is coverage: a file should be named where its content is relevant, so the reader meets the pointer at the point the file would help, and one term should mean one thing on both sides of the boundary. Report a coverage gap as Minor and name the line that should also name the file. That the body *is* the prompt sharpens the coverage question and not the wording one — a command frequently acts on its first turn, so a file whose content is never named where it is needed is unreachable in practice. Suggesting a fuller pointer on top of that is legitimate advice, but say in the finding that the form has no measured basis and leave the author the choice.
