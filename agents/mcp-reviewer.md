...
**Signposting (Minor; Critical when there is no pointer at all).** A pointer to a document should carry the condition that makes the reader open it, not merely its topic. "Read `docs/oauth-setup.md` before the first connection, because the server rejects an unregistered redirect URI" beats "see `docs/` for setup". The same rule applies inside a tool description: one that says "see the docs for parameters" has moved its schema somewhere the router cannot see it, and the router is the only reader that matters. Report a topical pointer as Minor with the condition written out; a document shipped beside the config that nothing points at is Critical.
